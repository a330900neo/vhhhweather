...
export default function HistoryPage() {
  ...
  // Helper to find the current hour label for the "NOW" line
  const currentHourLabel = new Date().toLocaleTimeString('en-HK', { 
  ...
  if (loading) {
    return (
      <div style={{
        ...
        GENERATING 36H TIMELINE (-24h to +12h)...
      </div>
    );
  }

  // Format X-Axis to only show labels every 4 hours to prevent mobile clutter
  ...
  return (
    <main style={{ 
      ...
      <h2 style={{ fontSize: '18px', margin: '15px 0' }}>VHHH 36-HOUR PERFORMANCE</h2>
      <p style={{ fontSize: '10px', color: '#556', marginBottom: '15px' }}>
        GREEN = ACTUAL (METAR) | BLUE = FORECAST (TAF)
      </p>
      ...
      {/* RAW DATA TABLE */}
      <h3 style={{ fontSize: '14px', marginTop: '30px', color: '#88a' }}>DETAILED LOG</h3>
          ...
          <tbody>
            {[...data].reverse().filter(d => d.raw).map((row: any, i: number) => (
              <tr key={i} style={{ borderBottom: '1px solid #162540' }}>
                ...
                <td style={{ color: '#ccc', fontSize: '9px', paddingRight: '15px' }}>{row.raw}</td>
              </tr>
            ))}
          </tbody>
        </table>
